...
var getAllowCount = (joshiName, allowCountOption) => {
    ...
    if (typeof allowCountOption === "object" && allowCountOption !== null) {
        var count = allowCountOption[joshiName];
        if (count !== undefined) {
...
var report = function report(context) {
    ...
    var { Syntax, report, RuleError, locator } = context;
            ...
            var checkSentence = /*#__PURE__*/ (function () {
              ...
              */
                    Object.keys(joshiTokenSurfaceKeyMap).forEach((key) => {
                        ...
                        var allowCount = getAllowCount(joshiName, options.allowCount);
                        if (joshiTokenSurfaceTokens.length <= allowCount) {
                            return; // no duplicated token exceeding allowCount
                        }

                        for (var i = allowCount; i < joshiTokenSurfaceTokens.length; i++) {
                            var prev = joshiTokenSurfaceTokens[i - 1];
                            var current = joshiTokenSurfaceTokens[i];
